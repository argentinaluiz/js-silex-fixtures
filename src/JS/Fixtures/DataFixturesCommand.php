...
class DataFixturesCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addArgument(
                'fixtures-path', InputArgument::REQUIRED, 'The directory or file to load data fixtures from.'
            )
            ->addOption(
                'append', null, InputOption::VALUE_NONE, 'Append the data fixtures instead of deleting all data from the database first.'
            )
            ->addOption(
                'purge-with-truncate', null, InputOption::VALUE_NONE, 'Purge data by using a database-level TRUNCATE statement'
            )
            ->setHelp(
                <<<EOT
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $loader = new ApplicationLoader($this->app);
